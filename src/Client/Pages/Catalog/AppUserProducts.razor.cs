    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (AppDataService != default)
        {
            if (AppDataService.AppUser != default)
            {
                ...
                var appUserProducts = (await AppUserProductsClient.GetByAppUserIdAsync(AppDataService.AppUser.Id)).ToList();
                ...
                Context = new(
                       ...
                       searchFunc: async filter =>
                       {
                           ...
                           productFilter.AppUserId = AppDataService.AppUser.Id;
                       ...
                       createFunc: async prod =>
                       {
                           ...
                           bool canCreate = await CanDoActionAsync(Context.CreateAction, state);

                           if (!canCreate && !prod.Product.ProductType.Equals(ProductType.Custom))
                           {
                               throw new Exception("You are authorized to access this resource. Testing");
                           }

                           prod.AppUserId = AppDataService.AppUser.Id;
                       ...
                       canUpdateEntityFunc: prod =>
                       {
                           ...
                           if (AppDataService.AppUser is not null && !string.IsNullOrEmpty(AppDataService.AppUser.RoleName))
                           {
